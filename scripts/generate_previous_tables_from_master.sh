...
echo ""
added_tables=$(git diff --name-only --diff-filter=A origin/master..origin/$1  -- 'datasets/**/v[0-9]*.json')
modified_tables=$(git diff --name-only --diff-filter=M origin/master..origin/$1  -- 'datasets/**/v[0-9]*.json')
